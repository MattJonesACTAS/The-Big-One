    ...
    <div className="h-screen bg-neutral-100 flex flex-col p-4 max-w-2xl mx-auto overflow-hidden relative">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-3 sm:mb-4">
        <button 
          ...
          className={`p-2 sm:p-4 rounded-xl text-xs sm:text-lg font-bold btn-base transition-colors ${state.currentOverlay === 'reversibles' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-700'} ${isShockForced ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
        >
        ...
        <button 
          ...
          disabled={isShockForced}
          className={`p-2 sm:p-4 rounded-xl text-xs sm:text-lg font-bold btn-base transition-colors ${state.currentOverlay === 'rosc' ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-700'} ${isShockForced ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
        >
        ...
        <button 
          ...
          disabled={isShockForced}
          className={`p-2 sm:p-4 rounded-xl text-xs sm:text-lg font-bold btn-base transition-colors ${state.currentOverlay === 'phea' ? 'bg-red-100 text-red-800' : 'bg-purple-100 text-purple-700'} ${isShockForced ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
        >
      ...
      <div className={`flex-1 bg-white border-4 rounded-3xl relative overflow-hidden transition-colors duration-300 ${
        ...
        <div className="h-full flex flex-col items-center justify-between p-4 relative">
          ...
          <div className="flex-1 flex flex-col items-center justify-center w-full pt-12 sm:pt-16">
            <div className="relative flex items-center justify-center w-[220px] h-[220px] sm:w-[310px] sm:h-[310px]">
              ...
              <div className="flex flex-col items-center z-10 translate-y-3 sm:translate-y-4">
                ...
                <div className="text-[8px] sm:text-[10px] text-neutral-400 uppercase tracking-widest font-bold mt-4 sm:mt-8">
                  Rhythm Check
                </div>
              </div>
            </div>
...
function CheckItem({ label }: CheckItemProps) {
  ...
  return (
    <label onClick={() => setChecked(!checked)} className={`flex items-center p-2 rounded-lg cursor-pointer transition-colors ${checked ? 'bg-emerald-50' : 'hover:bg-neutral-50'}`}>
      ...
      <span className={`text-[12px] font-medium ${checked ? 'text-emerald-900' : 'text-neutral-700'}`}>{label}</span>
    </label>
  );
}
...
function SectionGroup({ title, color, items }: { title: string, color: string, items: string[] }) {
  ...
  return (
    <>
      <div className={`p-2.5 px-4 z-10 font-bold text-[11px] tracking-wide border-b uppercase sticky top-0 text-center ${colorMap[color]}`}>{title}</div>
...
function TreatmentLog({ treatments, elapsedSeconds, catchupElapsed, isSummary = false }: { treatments: Treatment[], elapsedSeconds: number, catchupElapsed: number, isSummary?: boolean }) {
  return (
    <div className="bg-white rounded-b-xl border border-neutral-100 overflow-hidden shadow-sm">
      ...
      <div className={`grid ${isSummary ? 'grid-cols-[1.5fr_1fr_1fr]' : 'grid-cols-[1.5fr_1fr_1fr_1fr]'} bg-neutral-100 border-b border-neutral-200 px-4 py-3`}>
          ...
          [...treatments].reverse().map((tx, i) => {
            ...
            return (
              <div key={i} className={`grid ${isSummary ? 'grid-cols-[1.5fr_1fr_1fr]' : 'grid-cols-[1.5fr_1fr_1fr_1fr]'} px-4 py-4 items-center gap-1`}>
                <div className="text-[11px] font-bold text-neutral-900 pr-1 overflow-hidden text-ellipsis whitespace-nowrap" title={tx.name}>{tx.name}</div>
                <div className="text-[11px] text-neutral-800 font-medium tabular-nums text-center">{timeDisplay}</div>
                <div className="text-[11px] text-neutral-800 font-medium tabular-nums text-center">{elapsedDisplay}</div>
                {!isSummary && <div className="text-[11px] text-neutral-800 font-medium tabular-nums text-center">{ago}</div>}
              </div>
            );
          })
        )}
...
function StatRow({ label, value, color = "text-neutral-900" }: StatRowProps) {
  return (
    <div className="flex justify-between items-center p-2 px-3">
      <span className="text-neutral-500 text-[11px] font-medium">{label}</span>
